...
class Blockchain:
    ...
    def is_valid_transaction(self, transaction):
        ...
        """
        if transaction.sender == "0" * 64: # Reward transaction
            return True
            
        # Verify signature
        # 1. Check if public key matches sender address
        if transaction.public_key:
            # We need to verify that hash(public_key) == sender_address
            derived_address = KeyPair.public_key_to_address(transaction.public_key)
            if derived_address != transaction.sender:
                print(f"ERROR: Public key {transaction.public_key} does not derive to sender {transaction.sender}")
                return False
        
        # Verify transaction signature
        if not transaction.verify_signature(transaction.public_key):
             print(f"ERROR: Invalid signature for tx {transaction.tx_id}")
             return False
             
        # Check balance
        sender_balance = self.get_balance(transaction.sender)
        if sender_balance < transaction.amount + transaction.fee:
            print(f"ERROR: Insufficient balance for {transaction.sender}: {sender_balance}")
            return False
